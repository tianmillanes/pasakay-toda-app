...
/// Comprehensive stats summary widget for driver dashboard
class DriverStatsSummaryWidget extends StatelessWidget {
  final bool showPasaBuy;
  final bool showRides;

  const DriverStatsSummaryWidget({
    super.key,
...
class _TripStatsSection extends StatelessWidget {
  final String userId;
  final FirestoreService firestoreService;

  const _TripStatsSection({
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<List<RideModel>>(
      stream: firestoreService.getUserRides(userId, isDriver: true),
      builder: (context, snapshot) {
        ...
        final completedTrips = trips.where((t) => t.status == RideStatus.completed).toList();
        final totalEarnings = completedTrips.fold<double>(0, (sum, trip) => sum + trip.fare);
        final thisMonthTrips = completedTrips.where((t) {
...
class _PasaBuyStatsSection extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return StreamBuilder<List<PasaBuyModel>>(
      stream: firestoreService.getDriverPasaBuyRequests(userId),
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return _StatsSkeletonLoader();
        }
        ...
        final totalEarnings = completedRequests.fold<double>(0, (sum, req) => sum + req.budget);
